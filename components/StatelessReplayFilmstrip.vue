...
/*
  ...
  before") plus exactly ONE new line (highlighted). The repeated tinted
  block visibly grows left→right. A token bar under each envelope climbs.
  The model answer is never pre-shown — it appears as the new highlighted
  line only once that call is sent.
...
// Build the three envelopes. Envelope k (0-based) carries system + turns 0..k,
// where turns 0..k-1 are "replayed" and turn k is "new" (incl. its answer).
const envelopes = computed(() =>
  script.map((_, k) => {
    const rows = [{ role: 'system', text: system, kind: 'replay' }]
    for (let i = 0; i <= k; i++) {
      rows.push({ role: 'user', text: script[i].u, kind: i < k ? 'replay' : 'new' })
      rows.push({ role: 'assistant', text: script[i].a, kind: i < k ? 'replay' : 'new' })
    }
...
<template>
  <div class="srC">
    ...
    <div class="legend">
      ...
      <span class="grow" :style="{ opacity: summary ? 1 : 0 }">
        ↗ the replayed block grows every call — you pay to resend it all
      </span>
    </div>
  </div>
</template>
...
.grow { color: var(--warm-bright); font-weight: 600; transition: opacity 0.4s ease; }
</style>
