...
/*
  NextTokenPredictorV2 — alternate take on "how a prompt becomes a prediction".
  Same D5 pipeline as NextTokenPredictor.vue, with the tweaks the user asked for:
    • PROMPT is now a QUESTION — "What's the best Salesforce acquisition?".
    ...
    • the first distribution is a CLOSE race — MuleSoft a touch ahead, Slack and
      ...
      fact" land harder.
    • the explanatory captions ("…written as a vector", "billions of these…")
      are larger, stronger-coloured and sit a little lower so they read.
...
<template>
  <div class="ntp">
    <svg class="net" viewBox="0 0 1100 400">
      ...
      <text class="cap top" :x="PROMPT_MID" y="80" text-anchor="middle" :class="{ on: bTokens }">
      ...
      <text class="cap" x="214" :y="nBottom + 34" text-anchor="middle" :class="{ on: bNumbers }">
      ...
      <g class="weights">
        ...
        <g v-if="bWeights" :key="sweepKey" :clip-path="'url(#wclipv2)'">
          <rect class="sweep" :x="WX" :y="WY" width="40" :height="wGridH" />
        </g>
        <clipPath id="wclipv2"><rect :x="WX" :y="WY" :width="wGridW" :height="wGridH" rx="3" /></clipPath>
      </g>
